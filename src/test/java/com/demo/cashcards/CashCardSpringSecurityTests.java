...
public class CashCardSpringSecurityTests {
    ...
    void shouldNotAllowTokensWithAnInvalidAudience() throws Exception{
        String token = mint((claims) -> {
            claims.audience(List.of("https://wrong"));
    ...
    void shouldNotAllowTokensThatAreExpired() throws Exception{
        String token = mint((claims) -> {
            claims
            .issuedAt(Instant.now().minusMillis(3600))
            .expiresAt(Instant.now().minusMillis(3599));
        });
        
        this.mvc.perform(get("/cashcards/100").header("Authorization", "Bearer "+token))
                .andExpect(status().isUnauthorized())
                .andExpect(header().string("WWW-Authenticate", containsString("Jwt: expiresAt must be after issuedAt")));
    }
